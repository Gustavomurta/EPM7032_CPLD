/* EPM7032S Parallel Programmer - ERASE Version 10
PLD Altera EPM7032SLC44 in PROG MODE 
Arduino IDE 2.3.6 - Arduino MEGA
Gustavo Murta - 2026_01_17

EDIT/VPP - PG0 (PIN41) = 12V VPP control
VCC - PG1 (PIN41) = +5V VCC control (Carlos' request)
...
void setup() {
  ...
  Serial.println("EPM7032S Parallel Programmer - ERASE V10");
...
void phase01EPM7032() {
  ...
  PORTG |= (1 << 1);      // set High +5V VCC ON (PG1) - (Carlos' request)
...
void eraseCPLD() {
  Serial.println("Erase CPLD");

  PORTL |= (1 << 4);       // set High MTIN (PL4)
  delayMicroseconds(200);  // delay 200 microseconds
  PORTL &= ~(1 << 4);      // set Low MTIN (PL4)
  delayMicroseconds(500);  // delay 500 microseconds

  PORTL |= (1 << 2);     // set High TM (PL2)
  delayMicroseconds(6);  // delay 5 microseconds
  PORTA = 0x3B;          // SET Address = 0x3B
  PORTL |= (1 << 1);     // set High BE (PL1)
  delayMicroseconds(6);  // delay 5 microseconds
  PORTL &= ~(1 << 0);    // set Low NTPW (PL0)
  delay(100);            // delay 100 miliseconds - ERASE PULSE

  PORTL |= (1 << 0);     // set High NTPW (PL0)
  delayMicroseconds(6);  // delay 5 microseconds
  PORTL |= (1 << 3);     // set High MT (PL3)
  delayMicroseconds(6);  // delay 5 microseconds
  PORTL &= ~(1 << 1);    // set Low BE (PL1)
  delayMicroseconds(6);  // delay 5 microseconds
  PORTL &= ~(1 << 3);    // set Low MT (PL3)
  delayMicroseconds(6);  // delay 5 microseconds
  PORTL &= ~(1 << 2);    // set Low TM (PL2)
  delayMicroseconds(6);  // delay 5 microseconds
}
...
void lastPhaseEPM7032() {
  ...
  PORTL &= ~(1 << 0);     // set Low NTPW (PL0)
  delayMicroseconds(50);  // delay 50 microseconds
  PORTG &= ~(1 << 1);     // set Low +5V VCC OFF (PG1) - (Carlos' request)
  delayMicroseconds(50);  // delay 50 microseconds
  PORTL = 0x00;           // SET all pins to LOW
  PORTC = 0x00;           // SET all pins to LOW
  PORTA = 0x00;           // SET all pins to LOW
...
void loop() {
  ...
  Serial.println();
  delay(5000);  // delay 500 miliseconds
}
